...
import { LogOut, Home, List, DollarSign, User, ChevronLeft, ChevronRight, Menu, Compass, ClipboardList } from 'lucide-react';
...
const WorkerLayout = () => {
  ...
  const navItems = [
    ...
    { path: '/worker/available-jobs', label: 'Explore', icon: Compass },
    { path: '/worker/my-jobs', label: 'Tasks', icon: ClipboardList },
    { path: '/worker/earnings', label: 'Wallet', icon: DollarSign },
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg md:text-2xl font-bold text-white truncate max-w-[150px] md:max-w-none">
                  Hi, {user?.name?.split(' ')[0] || 'Provider'}
                </h1>
                <p className="hidden md:block text-sm text-gray-400">Your service dashboard is ready.</p>
              </div>
            </div>
            <button
              onClick={handleLogout}
